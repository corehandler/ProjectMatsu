...
static void pushclosure(LexState* ls, FuncState* func, expdesc* v) {
    ...
    for (i = 0; i < func->f->nups; i++) {
        int mode = (func->upvalues[i].k == VLOCAL) ? 1 : 2;
        luaK_codeABC(fs, OP_CLUPVAL, mode, func->upvalues[i].info, 0);
    }
}
...
void luaK_self(LexState* ls, expdesc* v, FuncState* fs, expdesc key) {
    expdesc args;
    int numparams;

    const auto func_register = fs->freereg;
    luaK_reserveregs(fs, 2);

    expdesc oldv = *v;
    v->u.s.info = func_register;
    v->k = VNONRELOC;

    const auto line = ls->linenumber;
    switch (ls->t.token) {
    case '(': {
        if (line != ls->lastline)
            luaX_syntaxerror(ls, "ambiguous syntax (function call x new statement)");
    ...
    auto base = v->u.s.info;
    if (hasmultret(args.k))
        numparams = LUA_MULTRET;
    else {
        if (args.k != VVOID)
            luaK_exp2nextreg(fs, &args);
        numparams = fs->freereg - (base + 1);
    }

    v->u.s.info = oldv.u.s.info;
    v->k = oldv.k;

    luaK_codeABC(fs, OP_SELF, func_register, v->u.s.info, 0);
    luaK_codeABx(fs, OP_PSUEDOSELF, func_register, key.u.s.info);
    freeexp(fs, &key);
    v->u.s.info = func_register;
    v->k = VNONRELOC;

    init_exp(v, VCALL, luaK_codeABC(fs, OP_CALL, base, numparams + 1, 2));
    luaK_fixline(fs, line);
    fs->freereg = base + 1;
}
